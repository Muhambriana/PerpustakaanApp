...
class CreateBookViewModel(private val repository: LibraryRepository) : ViewModel() {
    ...
    fun createBook(token: String, title: String, author: String, publisher: String, publisherDate: String, stock: String, description:String, image: MultipartBody.Part?, pdf: MultipartBody.Part?){
        resourceCreateBook.postValue(MyEvent(MyResource.Loading()))
        val book = Book(null, title, author, publisher, publisherDate, stock, description)
