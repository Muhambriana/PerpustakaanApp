...
class LibraryRepository constructor(private val retrofitClient: RetrofitClient) {
    ...
    fun createBook(token: String, data: Book, image: MultipartBody.Part?, pdf: MultipartBody.Part?) = retrofitClient.create().postCreateBook(token, data, image, pdf)
    ...
    fun findUser(token: String, username: ModelUsername) = retrofitClient.create().findUser(token, username)
